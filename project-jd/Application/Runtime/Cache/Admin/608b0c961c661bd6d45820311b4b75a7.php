...
<div class="tab-div">
    ...
    <div id="tabbody-div">
        <form enctype="multipart/form-data" action="/index.php/Admin/Goods/edit/id/50.html" method="post">
                ...
                <tr>
                    ...
                    <td>
                        <select name="cate_id" id="">
                            ...
                            <?php if(is_array($cateDate)): foreach($cateDate as $key=>$val): ?><option <?php if($val['id'] == $data['cate_id']) echo 'selected';?> value="<?php echo ($val['id']); ?>"><?php echo str_repeat('-',3*$val['level']).$val['cate_name'];?></option><?php endforeach; endif; ?>
                        </select>
                        ...
                        <?php if($extCateDate):?>
                            <?php foreach($extCateDate as $v):?>
                                    <select name="ext_cate_id[]" id="">
                                        ...
                                        <option <?php if($v['cate_id'] == $val['id']) echo 'selected';?> value="<?php echo ($val['id']); ?>"><?php echo str_repeat('-',3*$val['level']).$val['cate_name'];?></option>
                                        <?php endforeach;?>
                                    </select>
                            <?php endforeach;?>
                        <?php else:?>
                    ...
                    <td>
                        ...
                        <!--<select name="brand_id" id="">-->
                            ...
                            <!--<?php if(is_array($brandDate)): foreach($brandDate as $key=>$val): ?>-->
                                <!--<option value="<?php echo ($val['id']); ?>" <?php if($val['id']==$data['brand_id']) echo 'selected';?>><?php echo ($val['brand_name']); ?></option>-->
            ...
            <table style="display: none" width="90%" class="btn-table" align="center">
                <tr>
                    <td>
                        <?php if(is_array($memberLevelDate)): foreach($memberLevelDate as $k=>$val): ?><strong><?php echo ($val['level_name']); ?>：</strong><input type="text" name="member_price[<?php echo ($val['id']); ?>]" value="<?php echo ($memberPiceDate[$k]['price']); ?>" size="20" />元<br><?php endforeach; endif; ?>
                    </td>
                </tr>
            </table>
            <!-- 商品属性 -->
            <table style="display: none" width="90%" class="btn-table" align="center">
                ...
                <tr>
                    <td>
                        <ul id="attr_list">
                            <?php
 $attrId = array(); foreach($goodsAttrDate as $v): if(in_array($v['attr_id'],$attrId)){ $opt = '-'; }else{ $opt = '+'; $attrId[] = $v['attr_id']; } ?>
                                <li>
                                    ...
                                    <?php if($v['attr_option_values']): $attr = explode(',',$v['attr_option_values']); ?>
                                        ...
                                        <select name="attr_value[<?php echo ($v['id']); ?>][]" >
                                            ...
                                            <?php foreach($attr as $val):?>
                                                <option <?php if($val == $v['attr_values']) echo selected;?> value="<?php echo ($val); ?>"><?php echo ($val); ?></option>
                                            <?php endforeach;?>
                                        </select>
                                    <?php else:?>
                                        <input type="text" name="attr_value[<?php echo ($v['attr_id']); ?>][]" value="<?php echo ($v['attr_values']); ?>">
                                    <?php endif;?>
                                </li>
                            <?php endforeach;?>
                        </ul>
                        ...
                        <ul id="old_goods_pic" style="list-style: none;">
                            <?php foreach($goodsPicDate as $v):?>
                                <li style="float: left;width: 150px;height: 150px;position: relative">
                                    <span class="picbtn" pic_id="<?php echo ($v['id']); ?>" style="position: absolute;right:0;top: -8px;font-size: 20px;cursor: pointer;color: red;background: rgba(255,255,255,.5);">✕</span>
