...
<div id="main-div">
    <div id="menu-list">
        <ul id="menu-ul">
            <?php $priModel = D('privilege'); $priDate = $priModel->getBtns(); foreach($priDate as $k=>$v): ?>
                <li class="explode" key="02_cat_and_goods" name="menu">
                    <?php echo $v['pri_name']?>
                    <ul>
                        <?php foreach($v['children'] as $v1):?>
                            <li class="menu-item"><a href="<?php echo U($v1['module_name'].'/'.$v1['controller_name'].'/'.$v1['action_name']);?>" target="main-frame"><?php echo ($v1['pri_name']); ?></a>
                        <?php endforeach;?>
                    </ul>
                </li>
            <?php endforeach;?>
            <?php if(session('id') == 1):?>
                <li class="explode" key="02_cat_and_goods" name="menu">
                    GII模块
                    <ul>
                        <li class="menu-item"><a href="<?php echo U('Gii/Index/Index');?>" target="main-frame">Gii代码生成</a>
                    </ul>
                </li>
            <?php endif;?>
        </ul>
